...
export const OneDriveProvider = {
  ...
  /** Upload a file */
  async uploadFile(token, file, parentId = null, onProgress) {
    const path = parentId ? `/me/drive/items/${parentId}:/${file.name}:/content` : `/me/drive/root:/${file.name}:/content`
    
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest()
      xhr.open('PUT', `${GRAPH}${path}`)
      ...
      xhr.onload = () => {
        ...
        else reject(new Error(`OneDrive upload failed: ${xhr.status}`))
      }
      xhr.onerror = () => reject(new Error('OneDrive upload network error'))
      xhr.send(file)
    })
  }
}
...
export async function refreshOneDriveToken(refreshToken) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}))
    throw new Error(err.error?.message || 'OneDrive token refresh failed')
  }
  return res.json()
}
