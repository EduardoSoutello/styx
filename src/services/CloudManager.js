...
class CloudManagerClass {
      ...
      if (data && data.accounts) {
        const remoteAccounts = data.accounts.map(acc => {
          if (acc.providerId === 'mega') {
            ...
            // Restore session if available locally (to avoid unnecessary refreshes)
            const local = this._accounts.find(a => a.id === acc.id)
            if (local && local.token) {
              acc.token = local.token
            }
          }
          return acc
        })
  ...
  async _getAuthParam(accountId) {
    const acc = this._accounts.find(a => a.id === accountId)
    if (!acc) throw new Error('Account not found')
    if (acc.providerId === 'mega') return acc.session
    
    await this.refreshTokenIfNeeded(accountId)
    return acc.token?.access || acc.token // handle both old string tokens and new objects
  }

  async refreshTokenIfNeeded(accountId) {
    const acc = this._accounts.find(a => a.id === accountId)
    if (!acc || acc.providerId === 'mega') return

    // If token is just a string (old format), we can't refresh it
    if (typeof acc.token === 'string') return

    const { access, refresh, expiresAt } = acc.token || {}
    if (!refresh) return

    // Refresh if expiring in less than 5 minutes
    const now = Date.now()
    if (expiresAt && expiresAt > now + 300000) return

    console.log(`Refreshing token for ${acc.providerId} (${acc.email})...`)
    try {
      let newData
      if (acc.providerId === 'dropbox') {
        const { refreshDropboxToken } = await import('./providers/DropboxProvider')
        newData = await refreshDropboxToken(refresh)
      } else if (acc.providerId === 'googledrive') {
        const { refreshGoogleToken } = await import('./providers/GoogleDriveProvider')
        newData = await refreshGoogleToken(refresh)
      } else if (acc.providerId === 'onedrive') {
        const { refreshOneDriveToken } = await import('./providers/OneDriveProvider')
        newData = await refreshOneDriveToken(refresh)
      }

      if (newData) {
        acc.token = {
          access: newData.access_token,
          refresh: newData.refresh_token || refresh, // some providers don't return a new refresh token
          expiresAt: Date.now() + (newData.expires_in * 1000)
        }
        this._notify()
      }
    } catch (e) {
      console.error(`Failed to refresh token for ${acc.id}`, e)
      acc.needsReauth = true
      this._notify()
    }
  }
  ...
  /**
   * Add or update a Google Drive account
   * tokenData: { access_token, refresh_token, expires_in }
   */
  async connectGoogleDrive(tokenData) {
    ...
    const token = typeof tokenData === 'string' ? accessToken : {
   ...
   */
  async connectDropbox(tokenData) {
    const accessToken = typeof tokenData === 'string' ? tokenData : tokenData.access_token
   ...
   */
  async connectOneDrive(tokenData) {
    const accessToken = typeof tokenData === 'string' ? tokenData : tokenData.access_token
  ...
  /** Mark an account as needing re-authentication (e.g. on 401 error) */
  markNeedsReauth(accountId) {
    const idx = this._accounts.findIndex(a => a.id === accountId)
    if (idx >= 0) {
      this._accounts[idx].needsReauth = true
      this._notify()
    }
  }
  ...
  async transferFile(sourceId, targetId, file, targetFolderId, onProgress) {
    ...
    // Normalize targetFolderId for providers that expect empty string for root (like Dropbox)
    let folderId = targetFolderId
    if (targetAcc.providerId === 'dropbox' && folderId === null) folderId = ''

    try {
      // 1. Download to Blob
      const authParam = await this._getAuthParam(sourceId)
      const blob = await sourceProvider.getFileBlob(authParam, file.id || file.path, file)
      ...
      // 2. Upload to target
      const targetAuthParam = await this._getAuthParam(targetId)
  ...
  async transferFiles(sourceId, targetId, files, targetFolderId, onProgress) {
    let totalProgress = 0
    const count = files.length
    for (let i = 0; i < count; i++) {
      await this.transferFile(sourceId, targetId, files[i], targetFolderId, (pct) => {
        const itemPct = (i + pct) / count
        onProgress?.(itemPct)
      })
    }
    onProgress?.(1.0)
  ...
  async createFolder(accountId, name, parentId = null) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
    return provider.createFolder(authParam, name, parentId)
  }

  async deleteFile(accountId, itemId) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
    // For Google Drive, we prefer trashing by default (handled in provider)
    return provider.deleteFile(authParam, itemId)
  }

  /** Bulk delete multiple files */
  async deleteFiles(accountId, itemIds) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
    // Run in sequence to avoid rate limits / complexity, though parallel is possible
    for (const id of itemIds) {
      await provider.deleteFile(authParam, id)
    }
  }

  async renameFile(accountId, itemId, newName) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
    return provider.renameFile(authParam, itemId, newName)
  }

  async uploadFile(accountId, file, parentId = null, onProgress) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
    return provider.uploadFile(authParam, file, parentId, onProgress)
  }

  async getDownloadUrl(accountId, itemId, fileObj) {
    const provider = this.getProvider(accountId)
    const authParam = await this._getAuthParam(accountId)
