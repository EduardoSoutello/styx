  ...
  if (showReset) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div>
          <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '0.25rem' }}>{t('auth.resetTitle')}</h3>
        ...
        {resetSent ? (
          ...
          <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            ...
            <button type="submit" className="primary" disabled={loading} style={{ width: '100%', justifyContent: 'center' }}>
              {loading ? <span className="spin-small" /> : t('auth.resetBtn')}
            </button>
          </form>
        )}
        <button
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <span style={{ fontSize: '0.72rem', opacity: 0.4, flexShrink: 0 }}>{t('auth.emailDivider')}</span>
        <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
      </div>
      ...
      <button type="submit" className="primary" disabled={loading || !email || !password} style={{ width: '100%', justifyContent: 'center' }}>
        {loading ? <span className="spin-small" /> : t('auth.login')}
      </button>
    </form>
  )
}
...
function RegisterTab({ onGoogleLogin }) {
  ...
  const confirmError = confirm && password !== confirm ? t('auth.passwordsNoMatch') : ''
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
        <span style={{ fontSize: '0.72rem', opacity: 0.4, flexShrink: 0 }}>{t('auth.emailDividerRegister')}</span>
        <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.08)' }} />
      </div>

      <ErrorBanner message={error} />

      <InputField label={t('auth.name')} icon={User} value={name} onChange={setName} placeholder="Seu Nome" />
      <InputField label={t('auth.email')} icon={Mail} type="email" value={email} onChange={setEmail} placeholder="seu@email.com" />

      <div className="input-group">
        <label><Lock size={12} /> {t('auth.password')}</label>
        <div style={{ position: 'relative' }}>
          <input
            type={showPass ? 'text' : 'password'}
            value={password}
      ...
      >
        {loading ? <span className="spin-small" /> : t('auth.createBtn')}
      </button>
    </form>
  )
}
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.78rem', opacity: 0.4, marginTop: '0.5rem', letterSpacing: '0.1em' }}>
            {t('auth.subtitle')}
          </p>
        </div>
          ...
          {['login', 'register'].map(tabKey => (
            <button
              ...
              style={{
                ...
                border: tab === tabKey ? '1px solid rgba(255,255,255,0.1)' : '1px solid transparent',
                borderRadius: '9px',
                fontWeight: tab === tabKey ? 700 : 500,
                opacity: tab === tabKey ? 1 : 0.5,
