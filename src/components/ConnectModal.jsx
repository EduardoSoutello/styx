...
const PROVIDERS_LIST = [
  {
    id: 'googledrive',
    authType: 'oauth',
    description: 'Conectar via conta Google',
    available: true
  },
  {
    id: 'dropbox',
    authType: 'oauth',
    description: 'Requer VITE_DROPBOX_CLIENT_ID',
    available: !!import.meta.env.VITE_DROPBOX_CLIENT_ID
  },
  {
    id: 'onedrive',
    authType: 'oauth',
    description: 'Requer VITE_ONEDRIVE_CLIENT_ID',
    available: !!import.meta.env.VITE_ONEDRIVE_CLIENT_ID
  },
  {
    id: 'mega',
    authType: 'credentials',
    description: 'Conectar com email e senha',
    available: true
  }
]
          ...
          <div className="connect-modal-header">
            <div>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 800 }}>
                {step === 'pick' ? 'Adicionar Conta' : 'Entrar no MEGA'}
              </h2>
              <p style={{ fontSize: '0.8rem', opacity: 0.5, marginTop: '0.2rem' }}>
                {step === 'pick' ? 'Escolha um serviço de nuvem' : 'Insira suas credenciais do mega.nz'}
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <form onSubmit={handleMegaSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                ...
                <button type="submit" className="primary" style={{ flex: 2, justifyContent: 'center' }} disabled={loading}>
                  {loading ? 'Entrando...' : 'Conectar MEGA'}
                </button>
              </div>
            </form>
          )}
